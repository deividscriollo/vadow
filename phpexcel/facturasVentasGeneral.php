...
$y = 7;
    ...
    $consulta1 = pg_query("select num_factura,fecha_actual,hora_actual,fecha_cancelacion,tipo_precio,forma_pago,tarifa0,tarifa12,iva_venta,descuento_venta,total_venta,identificacion,nombres_cli,nombre_empresa,id_factura_venta from factura_venta, clientes,empresa,usuario where factura_venta.id_cliente=clientes.id_cliente and factura_venta.id_empresa=empresa.id_empresa and usuario.id_usuario=factura_venta.id_usuario and factura_venta.estado= 'Activo' and fecha_actual between '$_GET[inicio]' and '$_GET[fin]' order by factura_venta.id_factura_venta asc");
    ...
    if ($contador > 0) {
        while ($row1 = pg_fetch_row($consulta1)) {
            if ($repetido == 0) {
                ...
                $objPHPExcel->getActiveSheet()->getStyle("B" . $y . ":N" . $y)->getFont()->setBold(true)->setName('Verdana')->setSize(10);
